# Adapt from https://github.com/PrincetonUniversity/multi_gpu_training/blob/main/02_pytorch_ddp/mnist_classify_ddp.py
import argparse
import torch
...
def main():
    ...
    if use_cuda:  
        cuda_kwargs = {'num_workers': 1,  # the env variable "SLURM_CPUS_PER_TASK" seems to require setting --cpus-per-task=8        # cpu-cores per task (>1 if multi-threaded tasks)
                       'pin_memory': True,
                       'shuffle': True}
    ...
    train_loader = torch.utils.data.DataLoader(dataset1,
                                               ...
                                               num_workers=1, #int(os.environ["SLURM_CPUS_PER_TASK"]),
                                               pin_memory=True)
